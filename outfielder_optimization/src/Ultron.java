...
public class Ultron {
  ...
  private static Double shift = 10.0;
  ...
  public static void main(String[] args) throws IOException {
    ...
    System.out.format("Approximate runtime: %.2f minutes\n", approxRuntime);
    findBest();
    System.out.println(best);


    Double end = System.nanoTime() / (Math.pow(10, 9));
    System.out.format("Runtime : %.2f minutes \n", (end - start)/60);
  }
  ...
  private static void setData() throws IOException {
    ...
    batter = new Batter("pete alonso", offensiveTeamName, "alonsoL.csv");
          ...
          for (double l = cfY - shift; l <= cfY + shift; l+=step) {
            for (double m = rfX - shift; m <= rfX + shift; m+=step) {
              for (double n = rfY - shift; n <= rfY + shift; n+=step) {
                Position position = new Position(i, j, k, l, m, n, batter, leftFielder, centerFielder, rightFielder, pitcher);
